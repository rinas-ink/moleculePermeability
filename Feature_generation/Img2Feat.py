...
# -*- coding: utf-8 -*-
"""
Created on Fri May 24 18:51:17 2024
...
#smileslist=[predict_SMILES(image) for image in imagelist]
#print("Generated list of SMILES.")



#alternate code to obtain smiles from a csv file
cleaned=pd.read_csv("cleaned_data.csv")

smileslist=[cleaned.loc[cleaned['IMAGE_path'] == img, 'SMILES'].values[0] for img in imagelist]



selfieslist=[]
for i, smiles_str in enumerate(smileslist):
    print("Attempting SELFIES generation for Image:", imagelist[i])
    try:
        selfieslist.append(sf.encoder(smiles_str))
    except:
        print(">>>>>>>> Not successful.")
        selfieslist.append("NA")
print("Generated list of selfies.")
...
chosen_descriptors = ['BalabanJ', 'BertzCT', 'Chi0', 'Chi0n', 'Chi0v', 'Chi1', 
                      ...
                      'SlogP_VSA10', 'SlogP_VSA11', 'SlogP_VSA12', 'SlogP_VSA2', 
                      ...
                      'fr_piperzine', 'fr_priamide', 'fr_prisulfonamd', 'fr_pyridine', 
...
for i, smiles_str in enumerate(smileslist):
    print("Processing molecule file: ",  imagelist[i], "\n")
    datadict['SMILES'].append(smiles_str)
    datadict['SELFIES'].append(selfieslist[i])
    datadict['img_file'].append(imagelist[i])
    mol=Chem.MolFromSmiles(smiles_str)
    descriptor_values=list(mol_descriptor_calculator.CalcDescriptors(mol))
    for j, descriptor in enumerate(chosen_descriptors):
        datadict[descriptor].append(descriptor_values[j])
